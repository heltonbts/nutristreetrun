...
const AVATAR_COLORS = [
  '#5FB8A8', '#7B9FD4', '#C4845A', '#9B7FD4', '#5A9BC4', '#D4A05A',
];
...
function RunnerRow({ item }: { item: RunnerItem }) {
  ...
  return (
    <View style={[s.row, me ? s.rowMe : s.rowDefault]}>
      <Text style={[s.pos, { color: me ? colors.brandInk : item.pos <= 3 ? colors.brand : colors.textMute }]}>
        {item.pos}
      </Text>
  ...
  return (
    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      {isLoading ? (
        ...
        >
          {data?.myPos != null && (
            <Text style={s.myPos}>Você está em {data.myPos}º lugar</Text>
          )}
          {data?.items.map((item) =>
            tab === 'club' ? (
